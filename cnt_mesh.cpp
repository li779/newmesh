...
void cnt_mesh::add_tube_in_xz() {
	...
	for (int i = 0; i < my_tube1.bodies.size() - 1; ++i) {
		...
		btHingeConstraint* circleSpring = new btHingeConstraint(*b1, *b2, frameInA, frameInB);
		btHingeConstraint* circleSpring2 = new btHingeConstraint(*b3, *b2, frameInA, frameInB);
		btHingeConstraint* circleSpring3 = new btHingeConstraint(*b5, *b2, frameInA, frameInB);
		centerSpring->setLimit(
			-pi, // hingelowerbound
			pi, // hingeupperbound
		);
